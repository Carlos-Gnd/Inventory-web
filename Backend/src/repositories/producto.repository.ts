...
export class ProductoRepository {
  async listar(): Promise<Producto[]> {
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT p.IdProducto, p.Nombre, p.IdCategoria, c.Nombre as CategoriaNombre,
  ...
  async registrar(producto: Producto): Promise<number> {
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO Productos (Nombre, IdCategoria, Precio, Stock, Estado,
  ...
  async editar(producto: Producto): Promise<boolean> {
    const [result] = await pool.query<ResultSetHeader>(
      `UPDATE Productos SET Nombre = ?, IdCategoria = ?, Precio = ?, Stock = ?,
